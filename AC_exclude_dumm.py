...
class Environment():
    ...
    def __init__(self, L, T_MAX, T_UPDATE, n_actions, input_dims, hidden_dims, lr, gamma):
        ...
        self.observation_radius = (self.input_dims-1)/2 
        self.walker_position = random.randint(0, L-1) # pick random lattice site
        self.flip_coin = random.randint(0,1) # to determine whether [0,1] output from network will mean [1,-1] increments or [-1,1]
        self.walker_actor_critic = ActorCritic(input_dims, hidden_dims, n_actions, gamma) # assign walker a neural network
        self.walker_actor_critic.apply(weights_init_uniform) # uniformly distribute the weights and set biases to zero
        self.optimizer = T.optim.Adam(self.walker_actor_critic.parameters(), lr=lr) # give network an optimizer
        self.coin_for_actor = random.randint(0,2) # to determine whether [0,1] output from network will mean [1,-1] increments or [-1,1]
        self.coin_for_critic1 = random.randint(0,1) # feed observation as [-x,x] or as [x,-x] to critic
        self.coin_for_critic2 = random.randint(-self.lattice_size,self.lattice_size) # feed observation as [-x,x] or as [x,-x] to critic
        # record the data
    ...
    def run(self):
        ...
        for t_step in range(self.simulation_time):
            observation = np.zeros(self.input_dims) # access the state; here input_dims = 2 * observ_radius + 1 
            if self.coin_for_critic1 == 0:
                for i in range (self.input_dims):
                    if self.walker_position - self.observation_radius + i < 0:
                        observation[i] = self.coin_for_critic2 + self.walker_position - self.observation_radius + i + self.lattice_size
                    elif self.walker_position - self.observation_radius + i > self.lattice_size - 1:
                        observation[i] = self.coin_for_critic2 + self.walker_position - self.observation_radius + i - self.lattice_size
                    else:
                        observation[i] = self.coin_for_critic2 + self.walker_position - self.observation_radius + i
            else:
                for i in range (self.input_dims):
                    if self.walker_position + self.observation_radius - i < 0:
                        observation[i] = -self.coin_for_critic2 + self.walker_position + self.observation_radius - i + self.lattice_size
            ...
            delta_x, reward = 0, 0
            if self.coin_for_actor == 0: # to eliminate network bias
                if action == 0: # jump to the right
                    delta_x = 1
                else:           # jump to the left
                    delta_x = -1
            if self.coin_for_actor == 1:
                if action == 0: # jump to the right
                    delta_x = -1
                else:           # jump to the left
                    delta_x = 1
            ...
            if t_step > 0.9*self.simulation_time:
                ...
                self.record_pdf[self.walker_position] += 1/(0.1*self.simulation_time) # i don't think it matters whether I will be adding it up here or after the update
