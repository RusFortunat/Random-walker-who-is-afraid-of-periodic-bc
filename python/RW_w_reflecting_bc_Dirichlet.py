...
class Environment():
    ...
    def run(self):
        for t_step in range(self.simulation_time):
            ...
            # impose Dirichlet reflective boundary conditions
...
if __name__ == '__main__':
    ...
    pdf_and_current_filename = "RW_Refl_output/PDF_Dirichlet_Curret_L" + str(L) + "_TMAX" + str(T_MAX) + ".txt"
    np.savetxt(pdf_and_current_filename, np.c_[x_axis,pdf,current])

    # plot pdf 
    pdf_filename = "RW_Refl_output/PDF_Dirichlet_L" + str(L) + "_TMAX" + str(T_MAX) + ".png"
    ...
    # plot pdf shifted
    pdf_shifted_filename = "RW_Refl_output/PDF_Dirichlet_shifted_L" + str(L) + "_TMAX" + str(T_MAX) + ".png"
    plt.figure(figsize=(10,8))
    axes = plt.gca()
    plt.xlabel("X", fontsize=24)
    plt.ylabel("PDF", fontsize=24)
    ...
    current_filename = "RW_Refl_output/Current_Dirichlet_shifted_L" + str(L) + "_TMAX" + str(T_MAX) + ".png"
    plt.figure(figsize=(10,8))
    axes = plt.gca()
    plt.xlabel("X", fontsize=24)
